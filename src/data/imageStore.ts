...
const createImageDataFor = (imagePath: string, img: GalleryImage): Image => {
	...
	// Process EXIF data - convert date strings to Date objects
	let processedExif: ProcessedImageExif | undefined = undefined;
	if (img.exif) {
		processedExif = {
			focalLength: img.exif.focalLength,
			iso: img.exif.iso,
			fNumber: img.exif.fNumber,
			shutterSpeed: img.exif.shutterSpeed,
			model: img.exif.model,
			lensModel: img.exif.lensModel,
			// Convert captureDate string to Date object if present
			captureDate: img.exif.captureDate
				? typeof img.exif.captureDate === 'string'
					? new Date(img.exif.captureDate)
					: img.exif.captureDate instanceof Date
						? img.exif.captureDate
						: undefined
				: undefined,
		};
	}
	...
	return {
		...
		exif: processedExif,
		filmType: img.meta.filmType,
		analog: img.meta.analog,
	};
};
